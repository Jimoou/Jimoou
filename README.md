...
- **기간**: 2025.03 - 2025.05
- **기술**: Spring Batch, Java, MySQL, AWS S3
- **문제 상황**: R-JAVA/쉘스크립트/크론탭으로 운영되던 레거시 시스템의 불안정성
- **해결 과정**:
  - Spring Batch 기반 모듈형 아키텍처로 전면 재설계
  - PG사별 인터페이스 추상화로 확장성 확보
- **성과**:
  - PG사 추가 소요 시간 3주 → 1일로 95% 단축
  - 실패 Task 개별 재실행으로 전체 배치 처리 시간 70% 단축

</details>

<details>
<summary><b>실시간 정산/지급대행 Batch 재설계</b> - 트랜잭션 일관성 보장</summary>

- **기간**: 2025.02 - 2025.03
- **기술**: Spring MVC, Thread, Daemon, MariaDB
- **문제 상황**: 분리된 모듈로 인한 간헐적 이체 실패 및 민원 발생
- **해결 과정**: Thread/Daemon으로 순차적 실행 보장
- **성과**: 정산 지연 시간 10~13분 → 5분으로 단축, 가맹점 민원 대폭 감소

</details>

<details>
<summary><b>트랜잭션 무결성 보장 DAO 프레임워크 개발</b> (기여도 100%)</summary>

- **기술**: Java, Spring Transaction Manager, ThreadLocal
- **문제 상황**: 레거시 DAO의 트랜잭션 미보장 및 SQL 인젝션 취약점
- **해결 과정**: GenericDao 클래스 설계로 자체 DAO 라이브러리 개발
- **성과**: 금융 트랜잭션 무결성 보장 및 보안 취약점 해결
...
<details>
<summary><b>정산 시스템 재구축</b> - PHP에서 Spring Boot로 전환</summary>

- **기간**: 2024.04 - 2024.10
- **기술**: Spring Boot, JSP, jQuery, MySQL, AWS
- **주요 업무**:
  - 6단계 멤버십 수수료 구조 정규화
  - 가맹점 데이터 중복 생성 방지 시스템 구현
- **성과**: 관리 효율성 향상, 데이터 일관성 확보
